...
void CAutoWalls::TraceLine(Vector vecAbsStart, Vector vecAbsEnd, unsigned int mask, C_BasePlayer* ignore, trace_t* ptr) {
    Ray_t ray;
...
bool CAutoWalls::TraceToExit(Vector &end, trace_t *enter_trace, Vector start, Vector dir, trace_t *exit_trace) {
    ...
    while (distance <= 90.0f) {
        ...
        if (exit_trace->startsolid && exit_trace->surface.flags & SURF_HITBOX) {
            TraceLine(end, start, 0x600400B, (C_BasePlayer*)exit_trace->m_pEnt, exit_trace);
...
void CAutoWalls::ScaleDamage(int hitgroup, C_BasePlayer* enemy, float weapon_armor_ratio, float & current_damage) {
...
bool CAutoWalls::SimulateFireBullet(C_BasePlayer* pLocal, C_BaseCombatWeapon* pWeapon, FireBulletData &data) {
    if (!pLocal) {
        return false;
    }
    ...
    WeaponCSInfo_t* weaponData = GetCSWpnData(pWeapon);
    if (weaponData == NULL) {
        return false;
    }
...
float CAutoWalls::GetDamage(C_BasePlayer* LocalPlayer, C_BasePlayer* pEntity, int HitboxIndex) {
    Vector vMin, vMax;
    Vector dst = Math::ExtrapolateTick(GetHitboxPosition2(pEntity, HitboxIndex, vMin, vMax), pEntity->GetVelocity());
    //Vector dst = pEntity->GetPredictedPosition(HitboxIndex, vMin, vMax);
    
    return GetDamage(LocalPlayer, dst);
}

float CAutoWalls::GetDamage(C_BasePlayer* LocalPlayer, Vector HitboxPosition) {
    if (!LocalPlayer) {
        return 0.0f;
    }
...
float CAutoWalls::GetDamageByAngle(C_BasePlayer* LocalPlayer, Vector AimAngles) {
    if (!LocalPlayer) {
        return 0.0f;
    }
...
bool CAutoWalls::CanWallbang(C_BasePlayer* LocalPlayer, float &dmg) {
    ...
    WeaponCSInfo_t* weaponData = GetCSWpnData(ActiveWeapon);
    if (!weaponData) {
        return false;
    }
