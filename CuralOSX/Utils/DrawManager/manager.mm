...
void CImDrawings::DrawString(ImFont* drawFont, int x, int y, int font_flags, Color textColor, const char* textString, bool bCenter) {
    DrawString(drawFont, x, y, font_flags, textColor, bCenter, textString);
}

void CImDrawings::DrawString(ImFont* drawFont, int x, int y, int font_flags, Color clrColor, bool bCenter, const char* szText, ...) {
    if (!szText) {
        return;
    }
...
void CImDrawings::DrawRect(int x, int y, int w, int h, Color clrColor, float rounding) {
    ImGui::GetWindowDrawList()->AddRectFilled(ImVec2((float)(x), (float)(y)), ImVec2((float)(x + w), (float)(y + h)), clrColor.ToImColor(), rounding);
}

void CImDrawings::DrawOutlineRect(int x, int y, int w, int h, Color clrColor, float rounding, float thickness) {
    ImGui::GetWindowDrawList()->AddRect(ImVec2((float)(x), (float)(y)), ImVec2((float)(x + w), (float)(y + h)), clrColor.ToImColor(), rounding, thickness);
}

void CImDrawings::DrawLine(int x, int y, int x1, int y1, Color clrColor, float thickness) {
    ImGui::GetWindowDrawList()->AddLine(ImVec2((float)(x), (float)(y)), ImVec2((float)(x1), (float)(y1)), clrColor.ToImColor(), thickness);
}

void CImDrawings::Circle(Vector2D position, float points, float radius, Color color, float thickness) {
    ImGui::GetWindowDrawList()->AddCircle(ImVec2(position.x, position.y), radius, color.ToImColor(), (int)points, thickness);
}

void CImDrawings::DrawTriangle(int width, int height, Color clrColor, int x, int y, TriangleDirections direction, bool centered) {
...
void CImDrawings::FilledCircle(Vector2D position, float points, float radius, Color color) {
    ImGui::GetWindowDrawList()->AddCircleFilled(ImVec2(position.x, position.y), radius, color.ToImColor(), (int)points);
}

Vector2D CImDrawings::GetTextSize(const char* text, ImFont* font) {
    if (!font) {
        font = ImGui::GetFont();
    }
...
Vector2D CImDrawings::GetMiddle(float x, float y, float xOffset, float yOffset) {
...
void CImDrawings::SetDropShadowAlpha(float value) {
    this->dropshadow_alpha = (int)(value * 255.0f);
}

void CImDrawings::SetDropShadowAlpha(int value) {
    this->dropshadow_alpha = value;
}

std::shared_ptr<CImDrawings> DrawManager = std::make_unique<CImDrawings>();
